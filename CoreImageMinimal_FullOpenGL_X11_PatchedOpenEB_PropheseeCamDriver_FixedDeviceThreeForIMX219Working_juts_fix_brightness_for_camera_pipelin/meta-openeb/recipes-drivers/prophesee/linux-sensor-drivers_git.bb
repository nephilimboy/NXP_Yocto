...
# Use the exact package names that the kernel expects
...
# Main package depends on the kernel modules
...
do_install() {
    ...
    # Remove debug directories and files completely
    find ${D} -type d -name ".debug" -exec rm -rf {} + 2>/dev/null || true
    
    # Also strip debug symbols from the .ko files themselves
    find ${D} -name "*.ko" -exec ${STRIP} --strip-debug {} \;
}

# Create separate debug packages for the debug files
PACKAGES += "${PN}-dbg"
FILES:${PN}-dbg = "/usr/src/debug"

# Skip QA checks for buildpaths (common in kernel modules)
